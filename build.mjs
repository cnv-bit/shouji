...
const MANAGED_ARTIFACT_NAMES = [
    'yuzi-qq-only.bundle.js',
    'yuzi-qq-only.bundle.js.map',
    'yuzi-qq-only.bundle.css',
    'yuzi-qq-only.bundle.css.map',
...
const jsOutfile = resolve(outputDirectory, 'yuzi-qq-only.bundle.js');
const cssOutfile = resolve(outputDirectory, 'yuzi-qq-only.bundle.css');
...
if (isWatch) {
    ...
    console.log(`        ${displayRoot}yuzi-qq-only.bundle.js  ${formatBytes(jsSize)}`);
    console.log(`        ${displayRoot}yuzi-qq-only.bundle.css ${formatBytes(cssSize)}`);
}
